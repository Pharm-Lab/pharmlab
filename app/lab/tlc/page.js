...
const MODE_INFO = {
  ...
  solvent_front: { label: 'Step 2', desc: 'Click anywhere on the solvent front (top line)',            color: '#2563eb' },
...
export default function TLCPage() {
  ...
  return (
    <main style={{ maxWidth: '1040px', margin: '0 auto', padding: '2rem 1rem', fontFamily: 'sans-serif' }}>
      <Link href="/lab" style={{ fontSize: '13px', color: '#6b7280', textDecoration: 'none' }}>← Lab Prep</Link>
      <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#111827', margin: '1rem 0 4px' }}>TLC analyser</h1>
      ...
      {imageSrc && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {/* Step indicators */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 14px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', flexWrap: 'wrap' }}>
            {MODES.map((md, i) => {
              ...
              return (
                <div key={md} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <div style={{ width: '24px', height: '24px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? col : active ? col : '#e5e7eb', color: (done || active) ? 'white' : '#9ca3af', fontSize: '11px', fontWeight: '700', flexShrink: 0 }}>
                    {done ? '✓' : i + 1}
                  </div>
                  <span style={{ fontSize: '12px', color: active ? col : done ? '#6b7280' : '#9ca3af', fontWeight: active ? '600' : '400' }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                <span style={{ fontSize: '11px', color: '#374151', minWidth: '36px' }}>{Math.round(zoom * 100)}%</span>
              </div>
              <button onClick={() => { setImageSrc(null); setBaseline(null); setSolventFront(null); setSpots([]); setMode('baseline') }}
                style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #e5e7eb', background: 'white', fontSize: '12px', cursor: 'pointer', color: '#6b7280' }}>
                New image
              </button>
              <button onClick={exportPNG}
                style={{ padding: '5px 10px', borderRadius: '6px', border: 'none', background: '#111827', color: 'white', fontSize: '12px', cursor: 'pointer', fontWeight: '500' }}>
                ↓ Export
              </button>
            </div>
          </div>
          ...
          <div style={{ padding: '8px 14px', borderRadius: '8px', background: m.color + '12', border: `1px solid ${m.color}33`, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '12px', color: '#374151' }}>{m.desc}</span>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px', alignItems: 'start' }}>
            {/* Canvas */}
            <div style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '65vh', border: '1px solid #e5e7eb', borderRadius: '10px', background: '#1a1a1a', cursor: 'crosshair' }}>
              <canvas ref={canvasRef} onClick={handleClick} style={{ display: 'block' }} />
            </div>

            {/* Rf panel */}
            {baseline && solventFront && (
              <div style={{ minWidth: '160px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px' }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 10px' }}>Rf values</p>
                <div style={{ marginBottom: '8px', padding: '5px 8px', background: 'white', borderRadius: '6px', border: '1px solid #e5e7eb', fontSize: '11px', color: '#6b7280', display: 'flex', justifyContent: 'space-between' }}>
                  <span>Total dist.</span>
                  <span style={{ fontFamily: 'ui-monospace, monospace', fontWeight: '600', color: '#374151' }}>
                    {Math.abs(Math.round(baseline.y - solventFront.y))}px
                  </span>
                </div>
                {rfValues.length === 0
                  ? <p style={{ fontSize: '12px', color: '#9ca3af', margin: 0 }}>Click spots on the plate.</p>
                  : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      {rfValues.map(s => (
                        <div key={s.label} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '6px', padding: '7px 9px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                            ...
                            <span style={{ fontSize: '11px', color: '#374151' }}>Spot {s.label}</span>
                          </div>
                          <span style={{ fontSize: '16px', fontWeight: '700', color: '#7c3aed', fontFamily: 'ui-monospace, monospace' }}>{s.rf.toFixed(3)}</span>
                        </div>
                      ))}
                      <div style={{ padding: '5px 8px', background: '#0a0f1e', borderRadius: '5px' }}>
