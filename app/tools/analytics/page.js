...
function TechCard({ tech, compact = false }) {
  ...
  if (!t) return null
  return (
    <div style={{ background: t.bg, border: `1.5px solid ${t.border}`, borderRadius: '12px', padding: compact ? '10px 14px' : '16px 18px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: compact ? '4px' : '8px' }}>
        ...
        {!compact && <span style={{ fontSize: '12px', color: t.dark }}>{t.tagline}</span>}
      </div>
      {compact && <p style={{ fontSize: '12px', color: t.dark, margin: 0 }}>{t.tagline}</p>}
    </div>
  )
}
...
function DecisionTrainer() {
  ...
  const Q = ({ children, hint }) => (
    <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px', marginBottom: '8px' }}>
      <p style={{ fontSize: '14px', fontWeight: '500', color: '#111827', margin: '0 0 4px' }}>{children}</p>
      {hint && <p style={{ fontSize: '12px', color: '#9ca3af', margin: 0, lineHeight: 1.5 }}>{hint}</p>}
    </div>
  )
  ...
  const Btn = ({ onClick, active, color = '#2563eb', children }) => (
    <button onClick={onClick} style={{
      ...
      background: active ? color + '15' : 'white', color: active ? color : '#374151',
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <p style={{ fontSize: '12px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>Compound properties</p>
          {path.length > 0 && <button onClick={reset} style={{ fontSize: '12px', color: '#6b7280', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Reset</button>}
        </div>
      ...
      {/* Result */}
      <div>
        <p style={{ fontSize: '12px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>Recommendation</p>
        {!result ? (
          <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '2rem', textAlign: 'center', color: '#9ca3af' }}>
          ...
          <div>
            ...
            <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px', marginTop: '10px' }}>
              <p style={{ fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px' }}>Why this technique</p>
              <p style={{ fontSize: '13px', color: '#374151', margin: '0 0 10px', lineHeight: 1.65 }}>{result.reason}</p>
            </div>
  ...
  return (
    <div style={{ marginTop: '10px', border: `1px solid ${t.border}`, borderRadius: '10px', overflow: 'hidden' }}>
      <button onClick={() => setOpen(!open)}
        style={{ width: '100%', padding: '10px 14px', background: t.bg, border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px', fontWeight: '500', color: t.dark }}>
        ...
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {[
            ...
            <div key={item.label}>
              ...
              <p style={{ fontSize: '12px', color: '#374151', margin: 0, lineHeight: 1.65 }}>{item.content}</p>
            </div>
          ))}
        </div>
      )}
    ...
    <div>
        ...
        {Object.values(TECHNIQUES).map(tech => (
          <button key={tech.id} onClick={() => setSelected(tech.id)}
            style={{
              ...
              border: selected === tech.id ? `2px solid ${tech.color}` : '1px solid #e5e7eb',
              background: selected === tech.id ? tech.bg : 'white',
              color: selected === tech.id ? tech.dark : '#374151',
              transition: 'all 0.12s',
            }}>
            {tech.abbr}
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {/* Header */}
          <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '14px 16px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#374151', margin: 0, lineHeight: 1.7 }}>{t.plain}</p>
          </div>
          ...
          {[
            ...
            <div key={item.label} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px' }}>
              ...
              <p style={{ fontSize: '13px', color: '#374151', margin: 0, lineHeight: 1.65 }}>{item.content}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {t.detectors.map((d, i) => (
                <div key={i} style={{ fontSize: '12px', color: '#374151', padding: '4px 8px', background: 'white', borderRadius: '6px', border: '1px solid #f3f4f6' }}>
                  {d}
                </div>
              ))}
            </div>
          ...
          <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '12px 14px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#374151', margin: '0 0 6px', lineHeight: 1.65 }}>{t.pharmaExample}</p>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '4px' }}>
          {QUIZ_CASES.map((_, i) => (
            <div key={i} style={{ width: '24px', height: '4px', borderRadius: '2px', background: i < idx ? '#22c55e' : i === idx ? '#2563eb' : '#e5e7eb' }} />
          ))}
        </div>
        <span style={{ fontSize: '12px', color: '#6b7280' }}>Score: {score} / {idx}</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
            <p style={{ fontSize: '11px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px' }}>Compound to analyse</p>
            <p style={{ fontSize: '18px', fontWeight: '700', color: '#111827', margin: '0 0 10px' }}>{c.compound}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {Object.entries(c.properties).map(([k, v]) => (
                <div key={k} style={{ display: 'flex', gap: '8px', fontSize: '12px' }}>
                  <span style={{ color: '#9ca3af', textTransform: 'capitalize', minWidth: '90px', flexShrink: 0 }}>{k.replace('_', ' ')}</span>
          ...
          <p style={{ fontSize: '13px', fontWeight: '500', color: '#374151', marginBottom: '8px' }}>Select the most appropriate technique:</p>
              ...
              return (
                <button key={tech.id} onClick={() => !submitted && setSelected(tech.id)}
                  style={{
                    ...
                    border: isRight ? '2px solid #16a34a' : isWrong ? '2px solid #dc2626' : isSelected ? `2px solid ${tech.color}` : '1px solid #e5e7eb',
                    background: isRight ? '#f0fdf4' : isWrong ? '#fef2f2' : isSelected ? tech.bg : 'white',
                    color: isRight ? '#15803d' : isWrong ? '#991b1b' : isSelected ? tech.dark : '#374151',
                    transition: 'all 0.12s',
                  }}>
                  <span style={{ fontWeight: '600' }}>{tech.abbr}</span>
                  <span style={{ color: '#9ca3af', marginLeft: '8px', fontWeight: '400', fontSize: '11px' }}>{tech.name}</span>
          ...
          {!submitted
            ...
            : <button onClick={next}
                style={{ marginTop: '12px', width: '100%', padding: '10px', background: '#111827', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
        ...
        <div>
          {!submitted ? (
            ...
            <div>
              <div style={{ padding: '12px 14px', borderRadius: '10px', marginBottom: '10px', background: isCorrect ? '#f0fdf4' : '#fef2f2', border: `1px solid ${isCorrect ? '#bbf7d0' : '#fecaca'}` }}>
                <p style={{ fontSize: '14px', fontWeight: '600', color: isCorrect ? '#15803d' : '#dc2626', margin: '0 0 4px' }}>
                  {isCorrect ? '✓ Correct' : `✗ Incorrect — correct answer is ${TECHNIQUES[c.correct].abbr}`}
                </p>
              </div>
              <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px', marginBottom: '10px' }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 5px' }}>Why</p>
                <p style={{ fontSize: '13px', color: '#374151', margin: 0, lineHeight: 1.65 }}>{c.explanation}</p>
              </div>
              <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '12px 14px', marginBottom: '10px' }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 8px' }}>Full method</p>
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', gap: '8px', marginBottom: '4px', fontSize: '12px' }}>
                    <span style={{ color: '#9ca3af', minWidth: '110px', flexShrink: 0 }}>{row.label}</span>
...
export default function AnalyticsPage() {
  ...
  return (
    <main style={{ maxWidth: '1060px', margin: '0 auto', padding: '2rem 1rem', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#111827', margin: '0 0 4px' }}>Analytical Technique Trainer</h1>
        ...
        {TABS.map((t, i) => (
          <button key={i} onClick={() => setTab(i)}
            style={{
              ...
              border: 'none', borderBottom: tab === i ? '2px solid #2563eb' : '2px solid transparent',
              background: 'transparent', color: tab === i ? '#1d4ed8' : '#6b7280',
