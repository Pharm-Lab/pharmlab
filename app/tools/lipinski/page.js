...
function RuleBar({ rule, value }) {
  ...
  return (
    <div style={{ background: passes ? 'rgba(22,163,74,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${passes ? 'rgba(22,163,74,0.35)' : 'rgba(239,68,68,0.35)'}`, borderRadius: '10px', padding: '10px 12px', marginBottom: '8px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
        <span style={{ fontSize: '12px', fontWeight: '600', color: rule.color }}>{rule.name}</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: '14px', fontWeight: '700', color: passes ? '#86efac' : '#fca5a5' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'rgba(240,244,255,0.3)' }}>
...
export default function LipinskiPage() {
  ...
  const sliderRow = (label, value, setValue, min, max, step) => (
    <div style={{ marginBottom: '10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
        ...
        <span style={{ fontSize: '12px', fontFamily: 'ui-monospace, monospace', color: '#93b4f7', fontWeight: '700' }}>
  ...
  return (
    <main style={{ maxWidth: '1060px', margin: '0 auto', padding: '2rem 1rem', fontFamily: "'Inter',system-ui,sans-serif", background: '#0a0f1e', minHeight: '100vh', color: '#f0f4ff' }}>
      ...
      <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#f0f4ff', margin: '0 0 4px', letterSpacing: '-0.02em' }}>Lipinski & Drug-likeness</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '1.5rem', alignItems: 'start' }}>
          <div>
            <div style={{ background: '#0f1629', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
              <p style={{ fontSize: '11px', fontWeight: '600', color: 'rgba(240,244,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 12px' }}>Molecular descriptors</p>
            ...
            <div style={{ background: '#0f1629', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '12px 14px' }}>
              <p style={{ fontSize: '11px', fontWeight: '600', color: 'rgba(240,244,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 8px' }}>Load drug example</p>
              <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                {DRUG_EXAMPLES.slice(0, 6).map(ex => (
                  <button key={ex.name} onClick={() => loadExample(ex)}
                    style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '5px', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'rgba(240,244,255,0.65)' }}>
                    {ex.name}
                  </button>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {/* Overall verdict */}
            <div style={{ padding: '14px 16px', borderRadius: '12px', background: lipinskiPasses ? 'rgba(22,163,74,0.12)' : 'rgba(239,68,68,0.12)', border: `2px solid ${lipinskiPasses ? 'rgba(22,163,74,0.6)' : 'rgba(239,68,68,0.6)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontSize: '16px', fontWeight: '700', color: lipinskiPasses ? '#86efac' : '#fca5a5', margin: '0 0 3px' }}>
                ...
                <div style={{ textAlign: 'center', padding: '8px 16px', background: bcsColors[bcsClass] + '20', border: `1px solid ${bcsColors[bcsClass]}44`, borderRadius: '10px' }}>
                  <p style={{ fontSize: '10px', color: 'rgba(240,244,255,0.35)', margin: '0 0 2px' }}>Predicted BCS</p>
            ...
            {fails.length > 0 && (
              <div style={{ background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.35)', borderRadius: '10px', padding: '12px 14px' }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: '#fdba74', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px' }}>Formulation strategies to address violations</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {fails.filter(f => ['mw','logp','hbd','hba'].includes(f.id)).map(f => {
                    ...
                    return (
                      <p key={f.id} style={{ fontSize: '12px', color: 'rgba(240,244,255,0.7)', margin: 0, lineHeight: 1.55 }}>
      ...
      {tab === 'rules' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ background: '#0f1629', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '14px 16px', fontSize: '13px', color: 'rgba(240,244,255,0.65)', lineHeight: 1.7 }}>
          ...
          {RULES.map(rule => (
            <div key={rule.id} style={{ background: '#0f1629', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: '600', color: '#f0f4ff' }}>{rule.name}</span>
              </div>
              <p style={{ fontSize: '13px', color: 'rgba(240,244,255,0.7)', margin: '0 0 8px', lineHeight: 1.7 }}>{rule.why}</p>
              <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '8px', padding: '8px 12px' }}>
                <p style={{ fontSize: '11px', fontWeight: '600', color: 'rgba(240,244,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 3px' }}>Notable exceptions</p>
                <p style={{ fontSize: '12px', color: 'rgba(240,244,255,0.45)', margin: 0, lineHeight: 1.55 }}>{rule.exceptions}</p>
              </div>
            </div>
          ))}
      ...
      {tab === 'examples' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {DRUG_EXAMPLES.map(ex => {
            ...
            return (
              <div key={ex.name}
                ...
                style={{ background: '#0f1629', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '12px 16px', cursor: 'pointer', display: 'grid', gridTemplateColumns: '180px 1fr auto', gap: '12px', alignItems: 'center' }}>
                <div>
                  <p style={{ fontSize: '14px', fontWeight: '600', color: '#f0f4ff', margin: '0 0 2px' }}>{ex.name}</p>
                  <p style={{ fontSize: '11px', color: 'rgba(240,244,255,0.35)', margin: 0 }}>{ex.note}</p>
                </div>
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {[
                    ...
                    return (
                      <div key={d.label} style={{ padding: '2px 8px', borderRadius: '6px', background: p ? 'rgba(22,163,74,0.12)' : 'rgba(239,68,68,0.12)', border: `1px solid ${p ? 'rgba(22,163,74,0.35)' : 'rgba(239,68,68,0.35)'}` }}>
                        <span style={{ fontSize: '10px', color: 'rgba(240,244,255,0.35)' }}>{d.label} </span>
                        <span style={{ fontSize: '11px', fontWeight: '600', color: p ? '#86efac' : '#fca5a5' }}>{d.value}</span>
                      </div>
                    )
                  })}
                </div>
          ...
          <p style={{ fontSize: '12px', color: 'rgba(240,244,255,0.3)', textAlign: 'center', marginTop: '4px' }}>Click any drug to load its values into the calculator</p>
        </div>
      )}
    </main>
  )
