...
export const MDMA_INTERACTIONS = [
  ...
  color:     '#f97316',
  mechanism: 'Both have dissociative and serotonergic elements. Combined CNS depression with MDMA cardiovascular stimulation creates unpredictable physiological stress. Strong disorientation and loss of situational awareness. Difficult to assess own condition or seek help if needed.',
  },
  {
    substance: 'Alcohol',
    severity:  'MODERATE',
...
export const CANNABIS_INTERACTIONS = [
  ...
  severity:  'MODERATE',
  color:     '#f59e0b',
  mechanism: 'Cannabis can dramatically intensify ketamine dissociation. Unpredictable threshold shift — amounts of ketamine that would normally produce mild effects can trigger full dissociation when combined with cannabis. Many difficult ketamine experiences involve cannabis.',
  },
  {
...
export const COCAINE_INTERACTIONS = [
  ...
  },
  {
    ...
    severity:  'MODERATE',
    color:     '#f59e0b',
    mechanism: 'Opposing CNS effects — cocaine stimulates, ketamine dissociates. Combined cardiovascular strain. The stimulant can mask how dissociated you actually are, leading to additional dosing of either substance.',
  },
  {
    substance: 'Cannabis',
    severity:  'LOW–MODERATE',
...
export const AMPHETAMINE_INTERACTIONS = [
  ...
  severity:  'MODERATE',
  color:     '#f59e0b',
  mechanism: 'Combined cardiovascular strain from stimulant + anaesthetic. Amphetamine can mask ketamine sedation, leading to redosing. Disorientation and inability to assess own condition significantly increased.',
  },
  {
    substance: 'Cannabis',
    severity:  'MODERATE',
...
export const CATHINONE_INTERACTIONS = [
  ...
  severity:  'MODERATE–HIGH',
  color:     '#f97316',
  mechanism: 'Both have dissociative and cardiovascular effects. Combined CNS depression risk. Given the unreliable nature of cathinone market content, combining with ketamine adds a layer of unpredictability about what is actually interacting.',
  },
  {
...
// Sources: Yanagihara et al (2002), Malinovsky et al (1996),
//          Clements et al (1982), Fanta et al (2015),
//          Mion & Villevieille (2013), Morgan & Curran (2012)

export const KETAMINE_PK = {
  intranasal: {
    ka:    4.0,    // h⁻¹ — rapid nasal absorption, Tmax ~15-20 min
    F:     0.45,   // 45% intranasal bioavailability (Yanagihara 2002)
    Vd:    3.0,    // L/kg — large, lipophilic
    ke:    0.26,   // h⁻¹ — t½ ~2.7h
    thalf: 2.7,
  },
  oral: {
    ka:    0.8,    // h⁻¹ — slower GI absorption
    F:     0.16,   // ~16% oral bioavailability (extensive first-pass)
    Vd:    3.0,    // L/kg
    ke:    0.26,   // h⁻¹ — same elimination
    thalf: 2.7,
  },
}

// Effect / dissociation zones (ng/mL plasma ketamine)
export const KETAMINE_ZONES = [
  { min: 0,    max: 40,   label: 'Sub-threshold',                    color: '#22c55e', alpha: 0.06 },
  { min: 40,   max: 100,  label: 'Mild — analgesia, light dissoc.',   color: '#84cc16', alpha: 0.08 },
  { min: 100,  max: 200,  label: 'Moderate dissociation',             color: '#f59e0b', alpha: 0.10 },
  { min: 200,  max: 400,  label: 'Strong — k-hole territory',         color: '#ef4444', alpha: 0.14 },
  { min: 400,  max: 99999,label: '⚠ Anaesthetic dissociation',        color: '#7f1d1d', alpha: 0.20 },
]

export const KETAMINE_INTERACTIONS = [
  {
    substance: 'Alcohol / CNS depressants',
    severity:  'SEVERE',
    color:     '#dc2626',
    mechanism: 'Both cause CNS and respiratory depression. Ketamine alone maintains airway reflexes — combined with alcohol this protection is reduced. Risk of respiratory depression and aspiration (vomiting while dissociated). The most common cause of ketamine-related emergency presentations.',
  },
  {
    substance: 'Benzodiazepines / Z-drugs',
    severity:  'SEVERE',
    color:     '#dc2626',
    mechanism: 'Additive CNS depression and respiratory risk. Benzodiazepines are sometimes used to manage a difficult ketamine experience — this is a clinical intervention, not a recreational safety measure. Respiratory monitoring required if combined.',
  },
  {
    substance: 'Opioids',
    severity:  'SEVERE',
    color:     '#dc2626',
    mechanism: 'Additive respiratory depression. Ketamine is used clinically as an opioid-sparing analgesic precisely because it works differently — recreational combination loses this separation and stacks respiratory risk.',
  },
  {
    substance: 'MDMA / stimulants',
    severity:  'MODERATE',
    color:     '#f97316',
    mechanism: 'Cardiovascular strain — stimulant-driven tachycardia combined with ketamine. Psychological effects may be unpredictably intensified. Hyperthermia risk. Increased disorientation and loss of situational awareness.',
  },
  {
    substance: 'Cannabis',
    severity:  'MODERATE',
    color:     '#f59e0b',
    mechanism: 'Cannabis can intensify dissociation unpredictably and dramatically. Many people who have difficult ketamine experiences report cannabis involvement. Individual variation is very large.',
  },
  {
    substance: 'Tramadol',
    severity:  'MODERATE',
    color:     '#f59e0b',
    mechanism: 'Both have serotonergic activity. Combined serotonin syndrome risk. Tramadol also lowers seizure threshold — ketamine has proconvulsant properties at high doses, making the combination unpredictable.',
  },
]

export function simulateKetamine({
  doses,       // [{timeh, doseMg, route}]
  weightKg,
  tEnd = 12,
  nSteps = 2000,
}) {
  ...
  // Per-dose PK based on route
  const dosePK = sortedDoses.map(d => {
    const pk  = KETAMINE_PK[d.route ?? 'intranasal']
    const VdL = pk.Vd * weightKg
    return { ...pk, VdL }
  })
  ...
  for (let step = 0; step < nSteps; step++) {
    ...
    for (let i = 0; i < n; i++) {
      if (sortedDoses[i].timeh >= t && sortedDoses[i].timeh < tNext) {
        absorptionAmounts[i] += sortedDoses[i].doseMg * dosePK[i].F * 1000
      }
    }
    ...
    const f = (c, amounts) => {
      let absRate = 0
      for (let i = 0; i < n; i++) {
        absRate += (dosePK[i].ka * amounts[i]) / dosePK[i].VdL
      }
      return {
        dC:       absRate - dosePK[0].ke * c,
        dAmounts: amounts.map((a, i) => -dosePK[i].ka * a),
      }
    }
...
export function calcKetamineMetrics(pts) {
  if (!pts.length) return null
  const cmax    = Math.max(...pts.map(p => p.c))
  const cmaxPt  = pts.find(p => p.c === cmax)
  const cmaxIdx = pts.indexOf(cmaxPt)
  ...
  let clearTime = null
  for (let i = cmaxIdx; i < pts.length; i++) {
    if (pts[i].c < 40) { clearTime = pts[i].t; break }
  }

  const peakZone = KETAMINE_ZONES.slice().reverse().find(z => cmax >= z.min)
